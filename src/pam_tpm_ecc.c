 ...
 */
static const char *tpm_error_string(TSS2_RC rc) {
  ...
  if (tpm_rc & 0x0080) {
    ...
    switch (code) {
    case 0x0E:
      return "TPM: auth failure — wrong PIN or auth value"; /* TPM_RC_AUTH_FAIL
                                                             */
    case 0x0B:
      return "TPM: handle not found — key may have been evicted"; /* TPM_RC_HANDLE
                                                                   */
    case 0x1A:
      return "TPM: auth value or salt too long"; /* TPM_RC_VALUE */
    case 0x1B:
      return "TPM: scheme unsupported by key"; /* TPM_RC_SCHEME */
    }
  } else {
    /* TPM Format-0 errors — matched exactly */
    switch (tpm_rc) {
    case 0x0100:
      return "TPM: not initialized (missing Startup)"; /* TPM2_RC_INITIALIZE */
    case 0x0120:
      return "TPM: disabled"; /* TPM2_RC_DISABLED */
    case 0x0921:
      return "TPM: DA lockout — TPM locked, try again later"; /* TPM2_RC_LOCKOUT
                                                               */
    }
  }
  ...
  case 0x00080000U: /* TSS2_RESMGR_RC_LAYER */
    switch (rc & 0xFFFFU) {
    case 5:
      return "RM: bad reference — check key_handle and TCTI";
    }
    break;
  case 0x00070000U: /* TSS2_ESYS_RC_LAYER */
    switch (rc & 0xFFFFU) {
    case 0x000B:
      return "ESYS: parameter encryption failed";
    case 0x0018:
      return "ESYS: resource handle does not exist";
    }
    break;
  }
...
static void log_tss_error(pam_handle_t *pamh, const char *where, TSS2_RC rc) {
...
static void secure_munlock(void *p, size_t n) {
  if (p != NULL && n > 0)
    munlock(p, n);
}
...
static int open_and_validate_pubkey(pam_handle_t *pamh, const char *path) {
  struct stat st;
  int fd;
  ...
  if ((st.st_mode & 0777) & ~0644) {
    pam_syslog(pamh, LOG_ERR, "%s has overly-permissive mode %04o", path,
...
static int tpm_sign(pam_handle_t *pamh, const char *tcti_conf,
                    uint32_t key_handle_val, const unsigned char *pin,
                    size_t pin_len, const unsigned char *challenge,
                    size_t chal_len, unsigned char *sig_out, size_t *sig_len) {
  TSS2_RC rc;
  ...
  rc = Esys_TR_FromTPMPublic(ctx, (TPM2_HANDLE)key_handle_val, ESYS_TR_NONE,
...

  {
    ...
    memset(sig_out, 0, 64);
    memcpy(sig_out + 32 - rs, tpm_sig->signature.ecdsa.signatureR.buffer, rs);
    memcpy(sig_out + 64 - ss, tpm_sig->signature.ecdsa.signatureS.buffer, ss);
    *sig_len = 64;
  }
...
static int pubkey_verify(pam_handle_t *pamh, int pubkey_fd,
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc,
  ...
  /* ---- TPM signature ------------------------------------------- */
  if (tpm_sign(pamh, tcti_conf, key_handle, (unsigned char *)pin,
               strnlen(pin, MAX_PIN_LEN), challenge, CHALLENGE_SIZE, sig_raw,
               &sig_len) != 0) {
  ...
  if (pubkey_verify(pamh, pubkey_fd, challenge, CHALLENGE_SIZE, sig_raw,
                    sig_len) != 0) {
